"""
This code is reproduction of the code from the paper Deep Learning with Long Short-Term Memory for Time Series Prediction, with small adjustments.
...
def generate_mask_matrix(shape, connection=1.0):
    ...
    s[super_threshold_indices] = 1.0
    s[lower_threshold_indices] = 0.0
    return s


def generate_weight_mask(shape, connection=1.0):
...
class RCLSTMCell(nn.Module):
    ...
    def reset_parameters(self):
        ...
        self.mask_wih = torch.from_numpy(
            generate_weight_mask((self.input_size, self.hidden_size), self.connectivity)
        ).to(self.device)

        self.mask_whh = torch.from_numpy(
            generate_weight_mask((self.hidden_size, self.hidden_size), self.connectivity)
        ).to(self.device)
    ...
    def print_weight(self):
        print(self.weight_ih.data.numpy())
...
class RNN(nn.Module):
    ...
    def forward(self, input_, hx=None):
        ...
        for layer in range(self.num_layers):
            cell = self.get_cell(layer)
            layer_output, (layer_h_n, layer_c_n) = RNN._forward_rnn(cell=cell, input_=input_, hx=hx)
            input_ = layer_output
