from src.ETSformer.models import ETSformer
from src.ETSformer.data_provider.data_factory import data_provider
from src.ETSformer.exp.exp_basic import Exp_Basic
from src.ETSformer.utils.tools import EarlyStopping, adjust_learning_rate
from src.ETSformer.utils.metrics import metric
from src.ETSformer.utils.Adam import Adam
...
class Exp_Main(Exp_Basic):
    ...
    def train(self, setting):
        ...
        print(f"Training time: {self.train_time}s")
    ...
    def test(self, setting, data, save_vals=False):
        ...
        print("test shape:", preds.shape, trues.shape)
        trues_inversed_formatted = (trues.flatten().reshape(-1, 1) - test_loader.dataset.min_data) / (
            test_loader.dataset.max_data - test_loader.dataset.min_data
        )
        pred_inversed_formatted = (preds.flatten().reshape(-1, 1) - test_loader.dataset.min_data) / (
            test_loader.dataset.max_data - test_loader.dataset.min_data
        )

        self.metrics = get_prediction_metrics(trues_inversed_formatted.flatten(), pred_inversed_formatted.flatten())
